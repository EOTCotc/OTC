<template>
  <div class="content">
      ...
      <div class="person" @click="personCenter()">
        ...
        <div class="person_text">
          <div class="person_title">
            <p v-if="iskyc_text === $t('components.login.right.status[3]')">{{ uname }}</p>
            ...
            <van-button
              ...
              plain
              hairline
              >{{ $t('components.login.right.text[0]') }}:{{ item }}</van-button
            >
          </div>
    ...
    <div class="audit">
      <van-cell
        v-if="iskyc == 2"
        :title="$t('components.login.right.menu[0]')"
        is-link
      ...
      <van-cell :title="$t('components.login.right.menu[1]')" is-link @click="auditing('secondPhase')">
        <template #icon>
    ...
    <div class="audit">
      <van-cell :title="$t('components.login.right.menu[2]')" is-link @click="auditing('transaction')">
        <template #icon>
          <img src="@/static/icon/chart.png" alt="" />
        </template>
      </van-cell>
      <van-cell :title="$t('components.login.right.menu[3]')" is-link @click="auditing(1)">
        <template #icon>
          <i class="iconfont icon-xinshouyindao icon_left"></i>
        </template>
      </van-cell>
      <van-cell :title="$t('components.login.right.menu[4]')" is-link @click="auditing(2)">
        <template #icon>
          <i class="iconfont icon-changjianwenti icon_left"></i>
        </template>
      </van-cell>
      <van-cell :title="$t('components.login.right.menu[5]')" is-link @click="auditing('contact')">
        <template #icon>
          <i class="iconfont icon-lianxiwomen icon_left"></i>
        </template>
      </van-cell>
      <van-cell :title="$t('components.login.right.menu[6]')" is-link @click="auditing('feedback')">
        <template #icon>
...
export default {
  ...
  data() {
    return {
      ...
      list: [
        {
          title: this.$t('components.login.right.menu[7]'),
          icon: 'iconfont icon-gerenxinxi',
          childlist: [
            {
              title: this.$t('components.login.right.menu[8]'),
              iskyc: true,
              event: 'identity',
            },
            { title: this.$t('components.login.right.menu[9]'), event: 'receivingList' },
            { title: this.$t('components.login.right.menu[10]'), event: 'chain' },
          ],
        },
        {
          title: this.$t('components.login.right.menu[11]'),
          icon: 'iconfont icon-tuiguangxinxi',
          childlist: [
            { title: this.$t('components.login.right.menu[12]'), event: 'firstPhase' },
            { title: this.$t('components.login.right.menu[13]'), event: 'share' },
            { title: this.$t('components.login.right.menu[14]'), event: 'team' },
        ...
        // },
        {
          title: this.$t('components.login.right.menu[15]'),
          icon: 'iconfont icon-jiaoyiguanli',
          childlist: [
            { title: this.$t('components.login.right.menu[16]'), event: 'orderGather-full' },
            { title: this.$t('components.login.right.menu[17]'), event: 'order-Ticket' },
            { title: this.$t('components.login.right.menu[18]'), event: 'arbitration' },
            { title: this.$t('components.login.right.menu[19]'), event: 'arbitrator' },
            { title: this.$t('components.login.right.menu[20]'), event: 'important-userList' },
          ],
        },
      ],
      moneylist: [
  ...
  mounted() {
    ...
    this.name = asd.uname
    if (asd.item == this.$t('components.login.right.status[4]')) {
      this.item = 'A0'
    } else {
      this.item = asd.item
    }
    ...
    switch (asd.iskyc) {
      case '-1':
        this.iskyc_text = this.$t('components.login.right.status[0]')
        break
      case '0':
        this.iskyc_text = this.$t('components.login.right.status[1]')
        break
      case '1':
        this.iskyc_text = this.$t('components.login.right.status[2]')
        break
      case '2':
        this.iskyc_text = this.$t('components.login.right.status[3]')
        break
    }
  },
  methods: {
    ...
    go(data) {
      if (data.event != undefined) {
        if (this.iskyc == 2 && data.iskyc) {
          ...
          Toast.loading({
            message: this.$t('components.login.right.loading.text[0]'),
            forbidClick: true,
          })
        } else if (this.iskyc == -1 && data.iskyc) {
          ...
          if (data.event === 'order-Ticket') {
              ...
              this.$toast.warning(
                <div>
                  <p style="font-size:14px;margin:5px;color:red">
                    ${this.$t('components.login.right.loading.text[1]')}EOTC${this.$t('components.login.right.loading.text[2]')}
                  </p>
                  <p style="font-size:14px;margin:5px 0;">
                    EOTC${this.$t('components.login.right.loading.text[3]')}5000${this.$t('components.login.right.loading.text[4]')}
                  </p>
                </div>
              )
              return false
            ...
            if (getItem('myjifen') < 9) {
              this.$toast.warning(
                <div>
                  <p style="font-size:14px;margin:5px;color:red">
                    ${this.$t('components.login.right.loading.text[5]')}
                  </p>
                  <p style="font-size:14px;margin:5px">
                    ${this.$t('components.login.right.loading.text[6]')}10${this.$t('components.login.right.loading.text[7]')}
                  </p>
                </div>
              )
              return false
