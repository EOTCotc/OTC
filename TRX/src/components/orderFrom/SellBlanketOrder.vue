  ...
  <div class="content">
    ...
    >
      <div class="pop-content">
        <div class="pop-title">{{ $t("components.orderFrom.sorder_chushou") }} {{ orderItem.id }}</div>
        <div class="cells">
          <div class="cell">
            <p class="cell_title">{{ $t("components.orderFrom.order_price") }}</p>
            <div class="inputs">
              <input
                type="number"
                v-model="price"
          ...
          <div class="cell">
            <p class="cell_title">{{ $t("components.orderFrom.sorder_xianshou") }}</p>
            <div class="cell_flex">
        ...
        <div class="buttons">
          <p @click="cancel">{{ $t("components.orderFrom.order_cancel_order") }}</p>
        </div>
      </div>
    </van-popup>
    ...
    >
      <template #default>
        <div class="text_color">{{ $t("components.orderFrom.sorder_cancel") }}</div>
        <p
          :style="{ color: 'red', textAlign: 'center', margin: '0 0 15px 0' }"
        >{{ $t("components.orderFrom.order_huifu") }}</p>
      </template>
    </van-dialog>
    <!-- 取消订单数量 -->
    <van-dialog
  ...
  methods: {
    ...
    async addOrderNum_yes() {
      ...
      try {
        if (this.coinId != window.itself) {
          ...
          await Reconstruction_myApprove(usdtNum, this.Nowcoin.ads,this.coinId)
        } else {
    ...
    async cancel_yes() {
      try {
        if (this.close_Num * 1 > 0 && this.close_Num * 1 <= this.number) {
          ...
          this.$toast.warning(
            `${this.$t('components.orderFrom.sorder_input')}-${this.number}${this.$t(
              'components.orderFrom.sorder_input2'
            )}`
          )
        }
      } catch (err) {
    ...
    onNumInput(e) {
      ...
      if (Number(e.target.value) > max * 10) {
        ...
        this.$toast.clear()
        this.$toast.warning(`${this.$t('components.orderFrom.order_warn4')}${max * 10}`)
      }
      this.is_validVal()
    },
    //限购 最小 法币值
