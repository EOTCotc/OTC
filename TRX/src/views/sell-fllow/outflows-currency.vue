<template>
  <div class="order-pay-container">
        ...
        <ul>
          ...
          <template v-if="curpaymentterm[1] !== 'default'">
            <li v-if="curpaymentterm[1] !== 'xj'">
              ...
              <span v-if="cacheData.curpaymentterm === 'zfb'">{{ cacheData.sellerMthods.myalipay }}</span>
              <span
                v-if="cacheData.curpaymentterm === 'wx'"
          ...
          <main class="main">
            <template v-if="cacheData.item.cash === '0' || cacheData.item.cash === '1'">
              ...
              >
                ...
                <template #right-icon>
                  <van-icon name="success" v-if="curpaymentterm[1] == payType" />
                </template>
              </van-cell>
            </template>
    ...
    <van-dialog v-model="cancelOrderModel_show" :showConfirmButton="false" class="mask-infoTips"></van-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    handlecacheData() {
      if (this.item) {
        ...
        this.cacheData.badge_info = localStorage.getItem('csnum') !== '0' ? 1 : 0
    ...
    async handleEmitIssueMoney() {
      ...
      if (this.curpaymentterm[1] === 'default') {
        this.$toast.warning(<p style="font-size:16px;color:red">请选择您的收款方式</p>)
        return false
      }
      ...
      if (
        ...
        for (let i of coinList) {
          if (i.id == coinID) this.address = i.ads
        }
        try {
          ...
          await Reconstruction_sellOrder_user(oid, usdt.toString(), sj_ads, coinID)
