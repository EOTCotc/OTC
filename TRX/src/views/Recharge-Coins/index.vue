<template>
  <div class="zh-container" v-if="isVoild_token">
    <van-field v-model="token_val" center :placeholder="$t('views.recharge.index.placeholder')">
      <template #button>
        <van-button size="small" type="primary" @click="volidToken(token_val)">
          {{ $t("views.recharge.index.sure") }}
        </van-button>
      </template>
    </van-field>
  </div>
  <div class="zh-container" v-else>
    <p class="qb_bancle">{{ $t("views.recharge.index.wallet") }}：</p>
    <div class="balance">
      <div>
        <p>TRX: {{ myTrx }}</p>
      </div>
    ...
    <van-field
      readonly
      v-model="value"
      :label="$t('views.recharge.index.site')"
    ...
    <main>
      <div class="main">
        ...
        >
          <p>{{ $t("views.recharge.index.content[0]") }}</p>
          <p>{{ $t("views.recharge.index.content[1]") }}</p>
          <p>{{ $t("views.recharge.index.content[2]") }}</p>
          <p>{{ $t("views.recharge.index.content[3]") }}</p>
          <p>{{ $t("views.recharge.index.content[4]") }}</p>
          <p>{{ $t("views.recharge.index.content[5]") }}</p>
        </section>
      ...
      <van-row class="bottom" type="flex" justify="space-around">
        ...
        >{{ $t("views.recharge.index.estimate") }}：{{ total_trxServer() }}
        </van-col
        >
        <van-col span="10" :style="{ color: 'red', fontWeight: '700' }"
        >{{ $t("views.recharge.index.gross") }}：{{ total_coin() }}
        </van-col
        >
      </van-row>
    </main>
    ...
    <footer class="footer">
      <div class="cancel">
        <van-button type="default" @click="reharge">{{$t("views.recharge.index.recharge") }}</van-button>
      </div>
      <div class="next">
        <van-button type="default" :disabled="Transfers_Btn" @click="transfer">{{ $t("views.recharge.index.transfer") }}</van-button>
      </div>
    </footer>
    ...
    <van-dialog
      ...
      show-cancel-button
      @confirm="confirm"
      :confirmButtonText="type === 'transfer' ? $t('views.recharge.index.transfer') : $t('views.recharge.index.recharge')"
    >
      <template #title>
        <Reharge
          ref="numInput"
      ...
      if ((this.Transfers_val ?? "") !== "") {
        if (action_type === "recharge" && coin_type === "TRX") {
          if (parseFloat(val) > parseFloat(this.myTrx)) {
            this.$toast.error(this.$t("views.recharge.index.toast[0]"));
            return false;
          }
          await tcoinFee(val);
        ...
        if (action_type === "recharge" && coin_type === "USDT") {
          if (parseFloat(val) > parseFloat(this.myUsdt)) {
            this.$toast.error(this.$t("views.recharge.index.toast[1]"));
            return false;
          }
          try {
            await SendUSDT(
    ...
    async transfer() {
      if (
        ...
        try {
          ...
          if (data.State === "1") {
            await this.update_Data();
            this.$toast.success(this.$t("views.recharge.index.toast[2]"));
          }
        } catch (err) {
          this.$toast.error(err);
        }
      } else {
        this.$toast.error(this.$t("views.recharge.index.toast[3]"));
      }
    },
    jump() {
