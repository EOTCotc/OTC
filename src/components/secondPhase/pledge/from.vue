<template>
  <div>
    ...
    <div class="botton">
      <van-button block round color="#1B2945" @click="sumbit" :disabled="
          gross != '' && number != ''  ? false : true
        ">提交</van-button>
    </div>

    <van-popup round v-model="show" position="bottom" :style="{ height: '50%' }">
      <van-picker title="选择节点类型" show-toolbar :columns="columns" :default-index="2" @confirm="onConfirm" @cancel="onCancel" />
    </van-popup>
...
export default {
  ...
  methods: {
    sumbit() {
      StakingEotc({num:this.number}).then(res=>{
        let data=res.data
        if(data.State<0){
